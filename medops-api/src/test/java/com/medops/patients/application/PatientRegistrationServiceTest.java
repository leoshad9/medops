...
import com.medops.auth.domain.Role;
import com.medops.auth.domain.User;
import com.medops.auth.infrastructure.repository.RoleRepository;
import com.medops.auth.infrastructure.repository.UserRepository;
import com.medops.auth.security.jwt.JwtService;
import com.medops.auth.application.SessionResult;
import com.medops.auth.application.TokenIssuanceService;
...
class PatientRegistrationServiceTest {
    ...
    void registerPatient_createsUserAndProfile_andReturnsTokens() {
        ...
        when(tokenIssuanceService.issue(any(User.class)))
                .thenReturn(new SessionResult("access", "refresh", new UserInfo(UUID.randomUUID(), EMAIL, "PATIENT")));
